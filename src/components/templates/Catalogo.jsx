...
const Catalogo = () => {
  ...
  const obtener = async () => {
    const data = await fetchJsonp(
      "https://api.deezer.com/search/track?q=eminen&index=0&limit=37&output=jsonp"
    );
  ...
  return (
    <Fragment>
      ...
      <div className="resultado">
        <h1 className="text-red fs-1">Resultados</h1>
        <div className={classnames(classMusic)}>
          {artista.map((item, k) => (
            <div key={k} className="inf-musica">
              ...
              <h3 className="text-dark fw-bold fs-5">{item.title}</h3>
              <p className="text-gray">{item.artist.name}</p>
            </div>
          ))}
        </div>
      </div>
